...
grep -o 'GAATTC' data/Human_herpesvirus_1_strain_17/NC_001806.2.fasta | wc -l > ~/exercise01/result/result05_1.txt
# 2. Save the number of HincII recognition sites in the fasta file to "result05_2.txt".
grep -o -r 'GT[CT][AG]AC' data/Human_herpesvirus_1_strain_17/NC_001806.2.fasta | wc -l > ~/exercise01/result/result05_2.txt
# 3. Count the number of bases (A, T, G, C) in the fasta file and fill the "result05_3.csv" with the numbers.
awk -v FS="" 'NR>1 {for (i=1; i<=NF; i++) count[$i]++} END {printf("%d,%d,%d,%d\n", count["A"], count["T"], count["G"], count["C"])}' data/Human_herpesvirus_1_strain_17/NC_001806.2.fasta > ~/exercise01/result/result05_3.csv
